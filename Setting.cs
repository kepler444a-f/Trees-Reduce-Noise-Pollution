...
namespace TreesReduceNoisePollution
{
    ...
    public class Setting : ModSetting
    {
        ...
        // NEW: Controls the neighbor bleed radius (0 = current cell only, 1 = 3x3 grid, 2 = 5x5 grid, etc.)
        [SettingsUISlider(min = 0, max = 5, step = 1)]
        [SettingsUISection(kSection, "General")]
        public int AbsorptionRadius { get; set; } = 1;
        ...
        public override void SetDefaults()
        {
            ...
            AbsorptionRadius = 1; // Default to checking 1 cell out (3x3 grid)
            ReductionMode = 0;
            UpdateInterval = 128;
    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.AbsorptionRadius)), "Bleed-Over Effect Radius" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.AbsorptionRadius)), "0 = Only trees in the exact cell absorb noise. 1-5 = Trees in nearby cells can absorb noise with a distance falloff." },
